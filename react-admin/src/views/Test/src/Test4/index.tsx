...
const LazyLoad = ({ url, alt }: { url: string; alt: string }) => {
  const imgRef = useRef(null);

  const observeObj = new IntersectionObserver(
    (entries: any) => {
      if (entries[0].isIntersecting) {
        entries[0].target.src = entries[0].target.dataset.imgurl;
        observeObj.unobserve(imgRef.current);
      }
    },
    {
  ...
  useEffect(() => {
    if (imgRef.current) {
      observeObj.observe(imgRef.current);
    }
    // return () => {
    //   observeObj.unobserve(imgRef.current);
    // };
  ...
  return (
    <img ref={imgRef} className="h-200px w-300px" src="" data-imgurl={url} alt={alt} />
  );
};
...
export default () => {
  ...
  return (
    <>
      <div id="lazyLoadpage" className="overflow-scroll h-full mt-100 bg-pink">
        {imageList.map((i: string) => {
          return <LazyLoad key={i} url={i} alt={i} />;
        })}
      </div>
    </>
  );
